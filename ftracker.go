...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	calories := ((runningCaloriesMeanSpeedMultiplier * speedKmh * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMs, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	calories := (float64(lengthPool)*float64(countPool)/mInKm/duration + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return calories
}
